...
class L2SignatureTest: XCTestCase {
    ...
    let strTargetBundle = "sg.gov.tech.ApexSecurityFrameworkiOS"
    ...
    func testPrivateKeyLocal() {
        do {
            try SecurityUtil().getPrivateKeyFromP12(targetBundle: strTargetBundle, p12KeystoreName: strFileName, p12Key: strSecret)
        } catch let error {
            print(error)
    ...
    func testGetPublicKeyLocal() {
        do {
            try SecurityUtil().getPublicCertFromP12(targetBundle: strTargetBundle, p12KeystoreName: strFileName, p12Key: strSecret)
        } catch let error {
            print(error)
            XCTFail("Should not throw any exception during test execution")
        }
    ...
    func testL2Signature_1() throws {
        let privateKey:SecKey = try SecurityUtil().getPrivateKeyFromP12(targetBundle: strTargetBundle, p12KeystoreName: strFileName, p12Key: strSecret)
        do {
    ...
    func testL2_Verify_Signature_Test() throws{
        ...
        let publicCert:SecCertificate = try SecurityUtil().getPublicCertFromP12(targetBundle: strTargetBundle, p12KeystoreName: strFileName, p12Key: strSecret)
