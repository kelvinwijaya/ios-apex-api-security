...
//    try SecurityUtil().getPrivateKeyFromP12(p12KeystoreName: strFileName, p12Key: strSecret)
    ...
    let targetBundle = "sg.gov.tech.ApexSecurityFrameworkiOS"
    ...
    func testL1_Basic_Test() {
        ...
        do {
            let authorizationToken = try SecurityUtil().getToken(strRealm: realm, strAppId: appId, strSecret: secret, strURLNoPortNbr: url, strAuthPrefix: authPrefixL1, strHttpMethod: httpMethod, strNonce: nonce, strTimeStamp: timestamp, formList: [:], strPassword: nil, strAlias: alias, strFileName: nil, strTargetBundle: targetBundle)
    ...
    func testL2_Basic_Test() {
        ...
        do {
            let authorizationToken = try SecurityUtil().getToken(strRealm: realm, strAppId: appId, strSecret: nil, strURLNoPortNbr: url, strAuthPrefix: authPrefixL2, strHttpMethod: httpMethod, strNonce: nonce, strTimeStamp: timestamp, formList: [:], strPassword: passphrase, strAlias: alias, strFileName: fileName, strTargetBundle: targetBundle)
